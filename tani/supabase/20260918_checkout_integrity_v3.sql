-- TANI phase 1 — authoritative quote + checkout integrity.
-- Keeps v2 available for older clients and moves the current app to v3.

create schema if not exists private;

create or replace function private.checkout_quote_core(
  p_items jsonb,
  p_delivery_zones jsonb,
  p_lock_products boolean default false
)
returns jsonb
language plpgsql
security invoker
...
revoke all on function private.checkout_quote_core(jsonb,jsonb,boolean) from public, anon, authenticated;

create or replace function public.quote_cart_v2(
  p_items jsonb,
...
revoke all on function public.quote_cart_v2(jsonb,jsonb) from public, anon;
grant execute on function public.quote_cart_v2(jsonb,jsonb) to authenticated;

create or replace function public.checkout_create_order_group_v3(
  p_address_id uuid,
  p_phone text,
  p_notes text,
  p_items jsonb,
...
revoke all on function public.checkout_create_order_group_v3(uuid,text,text,jsonb,text,jsonb,numeric,text) from public, anon;
grant execute on function public.checkout_create_order_group_v3(uuid,text,text,jsonb,text,jsonb,numeric,text) to authenticated;
